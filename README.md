...
**在线体验：<https://void-space912.github.io/orbis-chronos/>**
...
└── scripts/
    ...
    ├── smooth.mjs        流畅度体检：像素比、绘制调用、2D 层每帧耗时、帧时间分布
    ├── langcheck.mjs     中英双语验收：两种语言下的文案对比与来回切换
...
node scripts/smooth.mjs      # 流畅度：像素比 / 绘制调用 / 特效层耗时
node scripts/langcheck.mjs   # 中英双语切换
...
| 优化 | 效果（`node scripts/smooth.mjs` 实测，1900 年快照） |
| --- | --- |
| **限制渲染分辨率**：globe.gl 默认按设备像素比最高 2 倍渲染，高分屏上是 4 倍像素量，已限制到 **1.5** | 模拟 dpr=2 时绘制缓冲区 1600×1000 → **2400×1500**（不限制会是 3200×2000，**像素少 44%**） |
| 占比小于 **0.4%** 的小地块不描边（它们的轮廓在球面上本来就看不见，却各占一次绘制调用） | 绘制调用 **2261 → 1934**（−14.5%） |
| 2D 特效层：72 根刻度合并成 2 条路径、刻度三角函数预计算、四角括号去掉 `save/rotate/restore` | 每帧 CPU 降到 **0.12–0.17 ms**（原先每帧 72 次路径绘制） |
| **拖动时间轴时临时关闭升降动画**（`polygonsTransitionDuration(0)`，松手恢复） | 快速滑动不再每换一个年代重播一次动画 |
| 升降过渡 1000 → **720 ms** | 过渡期间逐帧重建多边形几何的持续时间缩短 |
| 低帧率自检 | 中位帧时间 > 50 ms（低于约 20 fps）时，页面顶部给一次性建议（关描边 / 降速），十几秒后自动消失，**不擅自改用户设置** |
| 时间轴刻度只建一次、悬停高亮合并到同一帧、播放中不做闪光 | 见上一节 |

> 本机 headless 是软件渲染（SwiftShader），帧时间主要由光栅化决定，不能代表真机帧率；
> 上面给的是**像素量、绘制调用、每帧 CPU** 这类可比较的硬数字。
> 想知道自己机器上的表现：按下 F12 在控制台输入 `__orbis.state.timing` 可以看到中位帧时间。
