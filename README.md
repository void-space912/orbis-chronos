...
| 中英双语 | 右上角「中文 / EN」一键切换：界面文案、年代标注、政权名、大事记、档案卡片全部跟随；也可用 `?lang=en` 或 `#lang=en` 直接指定，选择记在 localStorage |
...
- **语言**：右上角切「中文 / EN」。切换是即时的——面板文案、球面上的政权名与大事标记、
  大事记条目、档案卡片都会换语言；中文界面下英文原名作为副标题，英文界面下中文名作为副标题。
  语言选择记在 `localStorage`，也可以用 `?lang=en`（或 `#y=1900&lang=en`）直接指定；
  查漏翻时会在控制台看到 `missing i18n key` 之类的提示（当前没有）。

### 翻译是怎么组织的

| 文件 | 内容 |
| --- | --- |
| `js/lang.js` | 界面文案词典（`zh` / `en` 两套，约 100 条）、`t('key', {n})` 取值、语言状态与切换事件 |
| `js/eras.js` | 54 个年代的 `label`/`labelEn`、`note`/`noteEn`，以及中英不同的年份格式（`公元前 1900 年` / `1900 BCE`） |
| `js/i18n.js` | 政权中文译名表（英文界面直接用数据集原名）、分类与「按洲」图例的中英标签 |
| `js/events.js` | 7 类历史大事的中英标签（事件名取 `data/events.json` 里的 `n` / `en`） |
| `index.html` | 静态文字用 `data-i18n` / `data-i18n-title` / `data-i18n-aria` 标记，由 `applyStaticI18n()` 统一刷新 |
